...
export function EventLog({ jobId }: EventLogProps) {
  const logs = useAppSelector(
    (s) => (s.training as { eventLogs: Record<number, LogEntry[]> }).eventLogs[jobId] ?? [] as LogEntry[],
  );
      ...
      <div className="overflow-y-auto max-h-72 px-6 py-3 flex flex-col gap-1.5 font-mono text-xs">
        {logs.map((entry, i) => (
          <div key={i} className="flex gap-3">
            <span className="text-subtle shrink-0">{entry.timestamp.split("T")[1]?.slice(0, 8)}</span>
